...
fn A(thetas: &DVector<f64>) -> DMatrix<f64> {
    let n = thetas.len();
    return DMatrix::from_fn(n, n, |r, c| (n - max(r, c)) as f64 as f64 * (thetas[r] - thetas[c]).cos());
}
...
fn b(thetas: &DVector<f64>, omegas: &DVector<f64>) -> DVector<f64> {
    ...
    for i in 0..n {
        let mut b_i = 0.;
        for j in 0..n {
            let coef = (n - max(i, j)) as f64;
            let delta = coef * (thetas[j] - thetas[i]).sin() * omegas[j] * omegas[j];
            b_i += delta;
        }
        b_i += G * (n - i) as f64 * thetas[i].sin();
        v.push(b_i)
    }
...
impl Pendula {
    ...
    #[wasm_bindgen(constructor)]
    pub fn new(ns_js: &JsValue, thetas_js: &JsValue) -> Pendula {
        let ns : Vec<usize> = ns_js.into_serde().unwrap();
        let thetas_vector : Vec<f64> = thetas_js.into_serde().unwrap();
        let mut pendula : Vec<Pendulum> = Vec::new();
        ...
        for n in ns {
            total_n += n;
            let theta = thetas_vector[theta_i];
            theta_i += 1;
            let thetas = DVector::from_vec(vec![theta; n]);
            let omegas = DVector::from_vec(vec![0.; n]);
